...
{	
	...
	//to stage
	robotNodeStagePub = nodeHandle.advertise<geometry_msgs::Twist>("robot_2/cmd_vel",1000);
	eventTriggerPub = nodeHandle.advertise<elderly_care_simulation::EventTrigger>("event_trigger", 1000, true);

	//subscribe to listen to messages coming from stage
	stageOdoSub = nodeHandle.subscribe<nav_msgs::Odometry>("robot_2/base_pose_ground_truth",1000, stageOdometryCallback);
	eventTriggerSub = nodeHandle.subscribe<elderly_care_simulation::EventTrigger>("event_trigger",1000, eventTriggerCallback);
    locationInstructionsSub = nodeHandle.subscribe<geometry_msgs::Point>("robot_2/location", 1000, updateDesiredLocationCallback);
    pathToRobotSub = nodeHandle.subscribe<std_msgs::Empty>("robot_2/toResident", 1000, goToResident);
    pathToHomeSub = nodeHandle.subscribe<std_msgs::Empty>("robot_2/toHome", 1000, goToHome);

...
const double WORLD_POS_Y = -3;
double px;
double py;
double theta;

unsigned int curTaskId;
unsigned int curTaskType;
...
bool TaskRequest_callback(	elderly_care_simulation::task_request::Request &req,
							elderly_care_simulation::task_request::Response &res)
{
	res.result = 1;
	ROS_WARN("ID: %d, TYPE: %d", req.id, req.type);
	return true;
...
ros::init(argc, argv, "AssistantRobo");
...
ros::ServiceServer task_request_service = n.advertiseService("task_request", TaskRequest_callback);
